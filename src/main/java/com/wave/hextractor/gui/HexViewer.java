...
public class HexViewer extends JFrame implements ActionListener {
    static {
        ...
        private void markOffsetStart(int absByte) {
            // Buscar offset cercano para extender (dentro de ±20 bytes, ambos lados)
            OffsetEntry targetEntry = null;
            if (offEntries != null) {
                for (OffsetEntry oe : offEntries) {
                    if (Math.abs(oe.getStart() - absByte) > 0 && Math.abs(oe.getStart() - absByte) <= OFFSET_SEARCH_RADIUS) {
                        targetEntry = oe;
                        break;
                    }
                }
            ...
            if (targetEntry != null) {
                if (Math.abs(targetEntry.getStart() - absByte) <= OFFSET_SEARCH_RADIUS) {
                    targetEntry.setStart(absByte);
                } else {
                    targetEntry.setEnd(absByte);
                }
                sortOffEntries();
            } else {
                currEntry.setStart(absByte);
            }
...
// ...existing code...
    ...
    private static final int OFFSET_SEARCH_RADIUS = 20;
    ...
    private void createFrame() {
        ...
        createMenu();
        // Wire actions and create auxiliary windows (search/new project)
        setActions();
        createSearchAllWin();
        createNewPrjWin();
        ...
        Action homeAction = new AbstractAction() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (currEntry != null && currEntry.getStart() >= 0 && currEntry.getEnd() < 0) {
                    currEntry.setStart(caretByteIndex);
                    refreshSelection();
                    updateOffsetLabel();
                } else {
    ...
    private void adjustWindowToContent() {
        ...
        setMinimumSize(new Dimension(Math.min(totalW, MIN_WINDOW_WIDTH), Math.min(totalH, MIN_WINDOW_HEIGHT)));
        revalidate();
        repaint();
    ...
    private void createNewPrjWin() {
        ...
        newPrjWin.pack();
        // Establecer tamaño fijo compacto
    ...
    private void createMenu() {
        ...
        // Comparar ROMs
        compareRomsItem = new JMenuItem("Comparar ROMs...");
        ...
        JMenuItem cols16 = new JMenuItem("16");
        JMenuItem cols32 = new JMenuItem("32");
        cols16.addActionListener(e -> changeColumns(16));
        cols32.addActionListener(e -> changeColumns(32));
        columnsSub.add(cols16);
        columnsSub.add(cols32);
        viewMenu.add(columnsSub);

        // Scale submenu items
        String[] scaleLabels = {"100%","125%","150%"};
        double[] scaleValues = {1.0,1.25,1.5,2.0};
        for (int i = 0; i < scaleLabels.length; i++) {
            JMenuItem scaleItem = new JMenuItem("Escala " + scaleLabels[i]);
            final double val = scaleValues[i];
            scaleItem.addActionListener(e -> {
                System.setProperty("hextractor.ui.scale", String.valueOf(val));
                applyScale();
            });
            viewMenu.add(scaleItem);
        }
        ...
        // Faltaban aceleradores: Ctrl+T (abrir tabla), Ctrl+S (guardar tabla), F3/F4 (vista)
    ...
    private void compareRomsAction() {
                ...
                UIManager.put("OptionPane.cancelButtonText", "Cancelar");
                UIManager.put("OptionPane.okButtonText", "Aceptar");
            // Cambiar textos de JFileChooser a español
            UIManager.put("FileChooser.saveInLabelText", "Guardar en:");
            UIManager.put("FileChooser.filesOfTypeLabelText", "Tipo de Archivo:");
            UIManager.put("FileChooser.fileNameLabelText", "Nombre:");
            UIManager.put("FileChooser.lookInLabelText", "Buscar en:");
        var cwd = System.getProperty("user.dir");
        JFileChooser chooser1 = new JFileChooser(cwd);
        ...
        javax.swing.filechooser.FileNameExtensionFilter romFilter = new javax.swing.filechooser.FileNameExtensionFilter(
            "ROMS",
            "md", "smd", "sms", "gba", "sfc", "nes", "bin", "smc", "gen", "gb", "gbc", "gg", "iso"
        );
        ...
        JFileChooser chooser2 = new JFileChooser(cwd);
        ...
        JFileChooser chooser3 = new JFileChooser(cwd);
        chooser3.setDialogTitle("Selecciona el archivo de salida .ext");
        // Default filename: TR_.<current folder name>.ext (note the dot after underscore)
        File cwdFile = new File(cwd);
        var folderName = cwdFile.getName();
        var defaultExtName = "TR_." + folderName + EXTENSION_EXTRACTION;
        chooser3.setSelectedFile(new File(cwd, defaultExtName));
        int res3;
        File outExt;
        while (true) {
        ...
        try {
            ...
            showMessage(MessageFormat.format(rb.getString(KeyConstants.KEY_EXTRACTION_COMPLETED), outExt.getAbsolutePath()), rb.getString(KeyConstants.KEY_SUCCESS_TITLE), JOptionPane.INFORMATION_MESSAGE);
        } catch (Exception ex) {
    ...
    private void setActions() {
        ...
        extract.setAction(new AbstractAction(rb.getString(KeyConstants.KEY_EXTRACT_MENUITEM)) {
            ...
            @Override
            public void actionPerformed(ActionEvent e) {
                var outFileName = tableFile.getName().replaceAll(FileUtils.getFileExtension(tableFile.getName()),
    ...
    private void setActionsNewPrjWin() {
        ...
        newPrjWinCreateButton.setAction(new AbstractAction(rb.getString(KeyConstants.KEY_NEW_PRJ_CREA_BUT)) {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                try{
                    var name = newPrjWinNameInput.getText();
                    var fileName = newPrjWinFileInput.getText();
                    // Usar longitud mínima fija (por ejemplo, 3)
                    formErrors = name == null || name.length() < 3 ||
                    ...
                    if(formErrors) {
                        ...
                        JOptionPane.showMessageDialog(newPrjWin,
        ...
        newPrjWinCancelButton.setAction(new AbstractAction(rb.getString(KeyConstants.KEY_NEW_PRJ_CLOSE_BUT)) {
            ...
            // After wiring actions, ensure accelerators remain set
            applyMenuAccelerators();
    }
    ...
    private void searchAllThreadAction() {
        ...
        final var extractFile = hexFile.getName() + Constants.EXTRACT_EXTENSION;
    ...
    private void buildThreeAreaViewer() {
        ...
        asciiTextArea.addMouseListener(new java.awt.event.MouseAdapter() {
            ...
            private void showPopupMenu(java.awt.event.MouseEvent e) {
                ...
                // Buscar si clic cae dentro de un OffsetEntry existente
                OffsetEntry tmpEntry = null;
                for (OffsetEntry oe : offEntries) {
                    if (clickAbsoluteByteIndex >= oe.getStart() && clickAbsoluteByteIndex <= oe.getEnd()) {
                        tmpEntry = oe;
                        break;
                    }
                }
                final OffsetEntry foundEntry = tmpEntry; // effectively final for lambdas

                JPopupMenu popup = new JPopupMenu();

                // Opción Inicio Offset (si no hay inicio marcado)
                ...
                if (currEntry.getStart() < 0) {
                    popup.add(startItem);
                }

                // Detectar si hay algún offset cuyo fin esté cerca del clic (para extender fin existente)
                // Permitir detección incluso si el clic cae dentro del propio offset
                final OffsetEntry nearEndEntry;
                if (offEntries != null) {
                    OffsetEntry tmpNear = null;
                    for (OffsetEntry oe : offEntries) {
                        int distEnd = Math.abs(clickAbsoluteByteIndex - oe.getEnd());
                        if (distEnd > 0 && distEnd <= 20) {
                            tmpNear = oe;
                            break;
                        }
                    }
                    nearEndEntry = tmpNear;
                } else {
                    nearEndEntry = null;
                }

                // Opción Fin Offset: mostrar si hay inicio marcado (para completar rango actual)
                // o si hay un offset existente con fin cercano (para extender fin de ese offset)
                if ((currEntry.getStart() >= 0 && clickAbsoluteByteIndex >= currEntry.getStart()) || nearEndEntry != null) {
                    JMenuItem endItem = new JMenuItem(rb.getString("offsetSetEnd"));
                    endItem.addActionListener(ev -> {
                        OffsetEntry targetEntry = nearEndEntry;
                        String result = JOptionPane.showInputDialog(HexViewer.this, rb.getString("inputEndchars"), lastSelectedEndChars);
                        if (result != null && result.length() > 0) {
                            lastSelectedEndChars = result;
                            if (targetEntry != null) {
                                targetEntry.setEndChars(Arrays.asList(result.toUpperCase().split("-")));
                                targetEntry.setEnd(clickAbsoluteByteIndex);
                                sortOffEntries();
                            } else {
                                currEntry.setEndChars(Arrays.asList(result.toUpperCase().split("-")));
                                currEntry.setEnd(clickAbsoluteByteIndex);
                ...
                if (foundEntry != null) {
                    ...
                    if (foundEntry.getEnd() - foundEntry.getStart() > 1) {
                        ...
                        splitItem.addActionListener(ev -> {
                            ...
                            if (valor != null) {
                                try {
                                    ...
                                    if (len > 0) {
                                        offEntries.remove(foundEntry);
                                        offEntries.addAll(foundEntry.split(len, fileBytes));
                                        sortOffEntries();
                                        refreshSelection();
                                    }
        ...
        asciiTextArea.addMouseWheelListener(wheelListener);
        hexTextArea.addMouseWheelListener(wheelListener);
        offsetsTextArea.addMouseWheelListener(wheelListener);
        ...
        am.put("hex_mark_end", new AbstractAction() { public void actionPerformed(ActionEvent e) {
            ...
            // Detectar offset con fin cercano (clic/pos caret hasta 20 bytes después del fin)
            OffsetEntry targetEntry = null;
            if (offEntries != null) {
                for (OffsetEntry oe : offEntries) {
                    int distEnd = Math.abs(absByte - oe.getEnd());
                    if (distEnd > 0 && distEnd <= 20) {
                        targetEntry = oe;
                        break;
                    }
            ...
            // Permitir completar currEntry si existe y el caret está después del inicio
            boolean canCompleteCurr = (currEntry.getStart() >= 0 && absByte >= currEntry.getStart());
            // Mostrar diálogo sólo si vamos a extender un offset existente o completar el actual
            if (targetEntry != null || canCompleteCurr) {
                String result = JOptionPane.showInputDialog(HexViewer.this, "Caracteres finales de cadena (FF-00-01...)", lastSelectedEndChars);
                if (result != null && result.length() > 0) {
                    lastSelectedEndChars = result;

                    if (targetEntry != null) {
                        // Extender offset existente hacia adelante
                        targetEntry.setEndChars(Arrays.asList(result.toUpperCase().split("-")));
                        targetEntry.setEnd(absByte);
                    } else {
                        // Completar currEntry
                        currEntry.setEndChars(Arrays.asList(result.toUpperCase().split("-")));
                        currEntry.setEnd(absByte);
                        currEntry.mergeInto(offEntries);
                        currEntry = new OffsetEntry();
                        currEntry.setStart(-1);
                        currEntry.setEnd(-1);
                ...
                if (files != null && files.length > 0) {
                    ...
                    for (File file : files) {
                        if (file.getAbsolutePath().endsWith(EXTENSION_TABLE)) {
                            reloadTableFile(file);
                        } else if (file.getAbsolutePath().endsWith(EXTENSION_OFFSET)) {
