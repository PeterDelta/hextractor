...
public class SMDChecksumUtils {
	...
	public static void checkUpdateMegaDriveChecksum(String inputFile) throws IOException {
		...
		Utils.log(Utils.getMessage("consoleFixingMegadriveChecksum", inputFile));
	...
	public static boolean checkUpdateMegaDriveChecksum(byte[] fileBytes) {
		...
		Utils.log(Utils.getMessage("consoleOriginalChecksum", Integer.toHexString(checksum).toUpperCase()));
		int calculatedChecksum = calculateMegaDriveChecksum(fileBytes);
		Utils.log(Utils.getMessage("consoleCalculatedChecksum") + ": 0x" + Integer.toHexString(calculatedChecksum).toUpperCase());
		boolean res = false;
