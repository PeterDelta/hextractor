...
public class GBChecksumUtils {
	...
	public static void checkUpdateGameBoyChecksum(String inputFile) throws IOException {
		byte[] fileBytes = FileUtils.readFileBytes(inputFile);
		Utils.log(Utils.getMessage("consoleFixingGameBoyChecksum", inputFile));
		...
		Utils.log(Utils.getMessage("consoleCalculatedChecksum") + " Header: 0x"
		...
		if ((calculatedHeaderChecksum & Constants.MASK_8BIT) != (headerChecksum & Constants.MASK_8BIT)) {
			...
			Utils.log(Utils.getMessage("consoleRomHeaderChecksumCorrect"));
		}

		// ROM CHECKSUM
		int romChecksum = getGameBoyRomChecksum(fileBytes);
		Utils.log(Utils.getMessage("consoleOriginalRomChecksum", Integer.toHexString(romChecksum).toUpperCase()));
		int calculatedRomChecksum = calculateGameBoyRomChecksum(fileBytes);
		Utils.log(Utils.getMessage("consoleCalculatedChecksum") + " ROM: 0x" + Integer.toHexString(calculatedRomChecksum).toUpperCase());
		if (calculatedRomChecksum != romChecksum) {
			Utils.log(Utils.getMessage("consoleUpdatingCalculatedChecksum"));
			updateGameBoyRomChecksum(calculatedRomChecksum, fileBytes);
			checksumModified = true;
		} else {
			Utils.log(Utils.getMessage("consoleRomChecksumCorrect"));
		}
		if (checksumModified) {
			Utils.log(Utils.getMessage("consoleWritingFile"));
