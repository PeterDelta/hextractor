...
public class FileUtilsTest {
	...
	public void getFileExtension() {
		...
		assertEquals("smc", FileUtils.getFileExtension(new File("Adventures of Tintin, The - Prisoners of the Sun (E).smc")));
	}
	...
	public void getFileWithDigests() throws IOException {
		...
		assertEquals("test.bin", testFileDigests.name());
		assertEquals("30a4e38230885e27d1bb3fd0713dfa7d", testFileDigests.md5());
		assertEquals("4e174bbc3e0a536aa8899d1f459318f797dc325a", testFileDigests.sha1());
		assertEquals("43549d77", testFileDigests.crc32());
		assertEquals(11, testFileDigests.bytes().length);
	}
	...
	public void getGameSystem() {
		...
		assertEquals(Constants.SYSTEM_SPV, FileUtils.getGameSystem("watara.sv"));
	}
	...
	public void getFileExtension1() throws IOException {
		...
		file.deleteOnExit();
		assertEquals("tst", FileUtils.getFileExtension(file.getAbsolutePath()));
	}
	...
	public void getFilePath() throws IOException {
		...
		file.deleteOnExit();
		assertNotNull(FileUtils.getFilePath(file));
	}
	...
	public void getAsciiFile() throws IOException {
		...
		assertEquals(ascii, FileUtils.getAsciiFile(file.getAbsolutePath()));
	}
	...
	public void extractAscii3To4Data() throws IOException {
		...
		assertEquals(decompStr, FileUtils.getAsciiFile(extFile.getAbsolutePath()));
	...
	public void insertHex4To3Data() throws IOException {
		...
		assertArrayEquals(data, Arrays.copyOfRange(Files.readAllBytes(outFile.toPath()), 0, data.length));
	}
	...
	public void insertHexData() throws IOException {
		...
		assertArrayEquals(data, Files.readAllBytes(hexFile.toPath()));
	}
	...
	@Test
	public void cleanAsciiFile() throws IOException {
		File extFile = File.createTempFile("table", ".ext");
		extFile.deleteOnExit();
		String asciiFile = "@00000000-00000004-FF\n" +
				...
				"|5\n" +
				";otro comment";
		FileUtils.writeFileAscii(extFile.getAbsolutePath(), asciiFile);
		File extFileClean = File.createTempFile("extFileClean", ".ext");
		extFileClean.deleteOnExit();
		FileUtils.cleanAsciiFile(extFile.getAbsolutePath(), extFileClean.getAbsolutePath());
		assertEquals("abcd\n", new String(Files.readAllBytes(extFileClean.toPath())));
	}
	...
	@Test
	public void checkLineLength() throws IOException {
		String asciiFile = "@00000000-00000004-FF\n" +
				";00008BB4{abcd~FF~}#028#022\n" +
				"abcd~FF~#005\n" +
				"|5";
		File checkFile = File.createTempFile("origin", "insertAsciiAsHex.tst");
		checkFile.deleteOnExit();
		FileUtils.writeFileAscii(checkFile.getAbsolutePath(), asciiFile);
		boolean ok;
		try {
			FileUtils.checkLineLength(checkFile.getAbsolutePath());
			ok = true;
		}
		catch(Exception e) {
			ok = false;
	...
	public void allFilesExist() throws IOException {
		String[] files = {"0", "1"};
		assertFalse(FileUtils.allFilesExist(files));
		...
		assertTrue(FileUtils.allFilesExist(files));
	}
	...
	public void replaceFileData() throws IOException {
		...
		assertEquals("0011111000", FileUtils.getAsciiFile(file.getAbsolutePath()));
	}
